...
- **Task 006: 데이터베이스 구축 및 RLS 정책 설정** ✅ - 완료
...
- **Task 007: 주최자 인증 시스템 구현 (F010)** ✅ - 완료
  - See: `/tasks/007-admin.md`
  - ✅ Supabase Auth 이메일 회원가입/로그인/로그아웃 연동 (기존 폼 컴포넌트 활용)
  - ✅ 소셜(Google) 로그인 연동 및 콜백 처리(`/auth/callback`) 및 next=/dashboard 명시
  - ✅ 미들웨어 기반 `/dashboard`, `/events/*`, `/profile` 보호 라우트 및 세션 갱신 처리
  - ✅ (organizer) layout 서버 컴포넌트에서 세션 체크 일괄 처리
  - ✅ 인증 UI 한국어 현지화 (로그인/회원가입/로그아웃 버튼, 에러 메시지)
...
- **Task 008: 이벤트 CRUD 및 초대 링크 API 구현 (F001, F002, F011)** ✅ - 완료
  - ✅ EventRepository → EventService → Server Actions 3계층 아키텍처 구현 (`lib/events/`)
  - ✅ 이벤트 생성/조회/수정/삭제 및 상태 전환(모집→확정→완료/취소) Server Action 구현
  - ✅ `crypto.randomUUID()` 기반 초대 토큰 자동 생성, VALID_TRANSITIONS 상태 전환 규칙 검증
  - ✅ 고유 토큰 기반 초대 링크 생성 및 토큰 검증 로직, 대시보드 목록/상태 필터 연동
  - ✅ 카카오톡 공유용 OG 메타 태그(`generateMetadata`) 적용 (`/join/[token]`)
  - ✅ `EventStatusChanger` 상태 전환 클라이언트 컴포넌트 신규 구현
  - ✅ 주최자 4개 페이지(events, events/[id], events/[id]/edit, dashboard) Mock → 실데이터 연동
  - **[테스트 필수 - 완료 조건]** Playwright MCP를 활용한 API 통합 테스트:
    - ✅ 이벤트 생성/수정/삭제 정상 동작 확인 — Server Action 3계층 구현 완료, tsc --noEmit 통과
    - ✅ 본인 소유 아닌 이벤트 수정/삭제 시 권한 차단 확인 — FORBIDDEN 에러 코드 + organizer_id 이중 조건
    - ✅ 잘못된 입력(필수값 누락, 과거 날짜 등) 검증 에러 확인 — Zod refine 검증 구현
    - ✅ 존재하지 않는 토큰 접근 시 404 처리 확인 — `/join/invalid-token-xyz` → 404 검증
    - ✅ 콘솔 에러 및 네트워크 오류 없음 확인 — Playwright browser_console_messages 검증
